...
export async function GET(req: Request) {
  ...
  if (url.searchParams.get('facets') === '1') {
    ...
    return NextResponse.json({facets}, {headers: CATALOG_CACHE_HEADERS})
  }

  const page = Math.max(1, Number.parseInt(url.searchParams.get('page') || '1', 10) || 1)
  const limit = Math.min(60, Math.max(1, Number.parseInt(url.searchParams.get('limit') || '30', 10) || 30))
  ...
  return NextResponse.json({items, total, page, limit}, {headers: CATALOG_CACHE_HEADERS})
}
