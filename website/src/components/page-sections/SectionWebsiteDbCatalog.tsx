...
import type {MarketingCatalogItemRow} from '@/lib/catalog/marketing-catalog-items'
import {
  fetchMarketingCatalogItemsByIds,
  fetchMarketingCatalogItemsFull,
  resolveCoverUrlsForItems,
...
import {WebsiteCatalogBrowse, type WebsiteCatalogBrowseItem} from '@/components/page-sections/WebsiteCatalogBrowse'
...
function rowToBrowseItem(
  ...
  extras?: Partial<Pick<WebsiteCatalogBrowseItem, 'displayTitle' | 'displaySubtitle' | 'objectPosition'>>,
...
export async function SectionWebsiteDbCatalog({section}: Props) {
      ...
      <section className={styles.section}>
        ...
        {missingEnv ? (
          <p className={styles.note}>
            Variables <code>NEXT_PUBLIC_SUPABASE_URL</code> et <code>SUPABASE_SERVICE_ROLE_KEY</code> manquantes : le
            catalogue Segna ne peut pas se charger.
          </p>
        ) : (
  ...
  if (mode === 'full_catalog') {
    const rows = await fetchMarketingCatalogItemsFull(200)
    const covers = await resolveCoverUrlsForItems(supabase, rows)
    const items = rows.map((r) => rowToBrowseItem(r, covers.get(r.id) ?? null))
    ...
    return (
      <section className={styles.section} aria-labelledby={heading ? `db-catalog-${section._key}` : undefined}>
        ...
        {items.length === 0 ? (
          <p className={styles.note}>
            Aucune pièce catalogue pour le moment. Vérifiez la migration SQL{' '}
            <code>get_marketing_website_catalog_items</code> et les pièces éligibles dans la base.
          </p>
        ) : (
          <WebsiteCatalogBrowse items={items} />
        )}
      </section>
    )
  }
  ...
  const browseItems: WebsiteCatalogBrowseItem[] = []
  ...
  return (
    <section className={styles.section} aria-labelledby={heading ? `db-catalog-${section._key}` : undefined}>
      ...
      <WebsiteCatalogBrowse items={browseItems} />
    </section>
  )
}
